...
void test_unload_distant_chunks()
{
    ...
    for (int x = -RENDER_DISTANCE; x <= RENDER_DISTANCE; x++)
    {
        ...
        {
            printf("Checking chunk at (%d, %d)\n", x, z);
            ClientChunk *chunk = GetChunk(&manager, x, z);
            if (chunk)
            {
                printf("Chunk at (%d, %d) still loaded\n", x, z);
            }
...
void test_reload_chunk()
{
    ...
    for (int x = -5; x <= 5; x++)
    {
        for (int z = -5; z <= 5; z++)
        {
            RemoveChunk(&manager, x, z);
            assert(GetChunk(&manager, x, z) == NULL && "Chunk should be successfully removed");
        }
    }
    FreeChunkManager(manager);
...
int main()
{
    ...
    while (1)
    {
        ...
        Rectangle button = {300, 250, 200, 100};
        int mouseOverButton = CheckCollisionPointRec(GetMousePosition(), button);

        // Change button color when hovered
        DrawRectangleRec(button, mouseOverButton ? LIGHTGRAY : GRAY);
        DrawText("START TESTS", 325, 285, 20, BLACK);

        // Check for button click
        if (mouseOverButton && IsMouseButtonReleased(MOUSE_LEFT_BUTTON))
        {
