...
// Modifions updateChunkMesh pour mieux gérer l'initialisation
static void updateChunkMesh(ChunkMesh *mesh, ChunkData *chunk)
{
    ...
    // Debug: afficher le nombre de faces
    int faceCount = 0;
    ...
    for (int x = 0; x < CHUNK_SIZE; x++)
    {
        for (int y = 0; y < WORLD_HEIGHT; y++)
        {
            for (int z = 0; z < CHUNK_SIZE; z++)
            {
                ...
                float shade = 0.85f + ((float)(rand() % 20) / 100.0f); // Variation de 0.8 à 1.2
